...
class AttendTrainingController extends Controller
{
    public function index(){
        return view('admin.attend_training.index');
    }

    public function store(Request $request){
        $booking = MemberTrainig::where('qrcode',$request->qrcode)->first();
        if(!$booking){
            return response(['status'=>false,'message'=>'رقم الكود غير موجود'],400);
        }

        if($booking->approve != 1 ){
            return response(['status'=>false,'message'=>'العضو غير مقبول'],400);
        }
    ...
    public function attendPerDay(){
        $workshos = AttendTraining::
        select('date','time', DB::raw('count(*) as total'))
            ->groupBy('date')->orderBy('date')->get();

        $count = AttendTraining::LeftJoin('member_trainigs' , 'member_trainigs.id' ,'attend_trainings.member_id')->count();

        return view('admin.attend_training.attendPerDay1',compact('workshos','count'));
    }



    public function BrowseAttendersTrainig($date){
        $results = AttendTraining::where('date',$date)->get();
        $route = '/admin/getBrowseEventAttendersData2/'.$date;
        $eventDate = $date;
        return view('admin.attend_training.browseAttenders',compact('results','route','eventDate'));
    }
    ...
    {
        if ($request->ajax()) {
            $data = AttendTraining::selectRaw('attend_trainings.id  , attend_trainings.created_at , member_id')->where('date',$date)->get();
            return Datatables::of($data)
                ...
                ->addColumn('name', function($row){
                    $member = MemberTrainig::where('id',$row->member_id)->first();
                    return $member->name;
                })
    ...
    public function attendAndPrintView(){
          return view('admin.attend_training.attendAndPrint');
    }
    ...
    public function storeByCode($code, Request $request)
    {
        ...
        $member = MemberTrainig::where('qrcode', $code)->first();
        $mytime = now();
        ...
        $check = AttendTraining::where('member_id', $member->id)
    ...
    public function saveAttendAndPrint(Request $request)
    {
        ...
        // البحث عن العضو
        $member = MemberTrainig::where('qrcode', $request->qrcode)->first();
