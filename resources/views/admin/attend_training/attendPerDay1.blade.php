        ...
        <div class="col-md-12">
            <div class="card shadow-base bd-0 overflow-hidden">
                <div class="card-header">
                    ...
                    <h5 class="card-title mt-2">إحصائيات الحضور التدريب</h5>
...
{{--                                <th> الساعة </th>--}}
                            ...
                            @foreach ($workshos as $key => $val)
                                <tr>
                                    ...
                                    <td>{{$val->date}}</td>
{{--                                    <td style="direction: ltr">{{ $val->time >= 12 ? ($val->time == 12 ? $val->time .' PM' : ($val->time - 12) .' PM' ) : ($val->time . ' AM')}}</td>--}}
{{--                                    <td>{{$val->time}}</td>--}}
                                    <td>{{$val->total}}</td>
                                    <td style="padding-left:1rem">
                                        <a title="عرض الزوار" class="btn btn-primary btn-sm" style="margin:.1rem" href="{{ url('admin/BrowseEventAttendersTrainig/'.$val->date) }}">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                        <a title="تصدير إلى إكسل" class="btn btn-success btn-sm" style="margin:.1rem" href="{{ url('admin/export/'.$val->date.'/2') }}">
